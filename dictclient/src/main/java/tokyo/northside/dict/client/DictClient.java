...
import tokyo.northside.dict.exceptions.DictError;
...
public class DictClient {
    ...
    /** Log class */
    private static final Logger LOGGER = LoggerFactory.getLogger(DictClient.class);
    ...
    private void check() throws DictError {
        if (socket == null) {
            throw new DictError(ResourceBundle.getBundle("tokyo/northside/dict/Messages")
    ...
    private String hash(String s) throws DictException {
        try {
            ...
            for (byte b : checksum) {
                String hs = Integer.toHexString((char) (b & 0xFF));
                if (hs.length() < 2) {
                    sb.append('0');
                }
                sb.append(hs);
