...
public class DictURLConnection extends java.net.URLConnection {
            ...
            try {
                ...
                if (method.equals("d")) {
                    ...
                    for (DefinitionResponse dr : client.getDefinitions(database, word)) {
                        tmp.append("From ").append(dr.getDbDescription()).append(" [").append(dr.getDatabase());
                        tmp.append("]:\n\n").append(dr.getTextualInformation()).append("\n\n");
                    }
                    body = tmp.toString().getBytes(StandardCharsets.UTF_8);
                }
                if (method.equals("m")) {
                    StringBuilder tmp = new StringBuilder();
                    for (Map.Entry<String, List<String>> entry : client.getMatches(database,
                            strategy, word).entrySet()) {
                        tmp.append("From ").append(entry.getKey()).append(":\n");
     ...
     *
     */
    private void setDefNum(String defNum) {
        if (defNum.length() > 0) {
            this.defNum = Integer.parseInt(defNum);
        }
    }
