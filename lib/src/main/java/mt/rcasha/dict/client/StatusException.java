...
public class StatusException extends DictException {
    ...
    public StatusException(int status, String text) {
        super(MessageFormat.format("{0}: {1}", //ResourceBundle.getBundle("org/dict/client/DictMessages").getString("StatusMessageFormat")
                new Object[] { Status.getMessage(status), text }));
