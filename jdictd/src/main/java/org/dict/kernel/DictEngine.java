...
public class DictEngine implements IDictEngine {
    ...
    public IAnswer[] define(String db, String word) {
        ...
        for (IAnswer an : ans) {
            if (an.getDefinition() != null) v.addElement(an);
        }
 ...
 */
    ...
    IAnswer[] defineMatchAll(String word, boolean define, int strategy) {
        ...
        for (IDatabase iDatabase : all) {
            IAnswer a = iDatabase.defineMatch(word, define, strategy);
            v.addElement(a);
        }
        IAnswer[] ret = new IAnswer[v.size()];
        v.copyInto(ret);
    ...
    protected IDatabase findDatabase(String id) {
        IDatabase[] all = getDatabases();
        for (IDatabase iDatabase : all) {
            if (iDatabase.getID().equalsIgnoreCase(id)) {
                return iDatabase;
            }
        }
        return null;
    ...
    public IAnswer[] lookup(IRequest req) {
        ...
        for (String s : db) {
            try {
                answers = this.defineMatch(s, word, pos, true, strat);
            } catch (Throwable t) {
    ...
    public synchronized void addDatabase(IDatabase db) {
        for (IDatabase fDatabase : fDatabases) {
            if (fDatabase.equals(db)) {
                return;
            }
        }
