...
public class HTMLPrinter implements IAnswerPrinter {
    ...
    public void appendLink(String s, String base, StringBuffer sb) {
        String str = s;
        try {
            str = new String(s.getBytes(StandardCharsets.UTF_8));
        } catch (Throwable t) {
        }
        sb.append("<a href=\"");
        sb.append(base);
    ...
    protected static String getOption(String db, String[] ids) {
        for (String id : ids) {
            if (db.equals(id)) return "\"" + db + "\" selected";
        }
        return "\"" + db + "\"";
    }

    public static void printAnswers(IDictEngine e, IRequest req, IAnswer[] a, boolean matches, PrintWriter out) throws Throwable {
        if (HEAD == null) {
            initialize();
        }
        out.println(HEAD);
    ...
    public static void printDefaultForm(IDictEngine e, IRequest req, PrintWriter out) throws IOException {
        ...
        for (IDatabase iDatabase : dbs) {
            String db = iDatabase.getName();
            out.println("  <option value=" + getOption(iDatabase.getID(), ids) + ">" + db);
        }
