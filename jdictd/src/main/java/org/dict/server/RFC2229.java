...
public class RFC2229 implements Runnable {
    ...
    int processDefine(String cmd) throws IOException {
        ...
        for (IAnswer iAnswer : m) {
            String id = iAnswer.getDatabase().getID();
            String name = iAnswer.getDatabase().getName();
            String res = iAnswer.getDefinition();
    ...
    int processMatch(String cmd) throws IOException {
        ...
        for (IAnswer iAnswer : m) {
            String id = iAnswer.getDatabase().getID();
            IWordPosition[] res = iAnswer.getAdjacentWords().getWordPositions();
            count += res.length;
            for (IWordPosition re : res) {
                sb.append(id + " \"" + re.getKey() + "\"");
                sb.append(CRLF);
            }
        }
        if (count == 0) {
    ...
    int processShowDB() throws IOException {
        ...
        for (IDatabase iDatabase : all) {
            sendLine(iDatabase.getID() + " \"" + iDatabase.getName() + "\"");
        }
        sendLine(".");
        sendLine("250 Command complete");
        return 0;
    ...
    int processShowInfo(String db) throws IOException {
        ...
        IDatabase it = null;
        for (IDatabase iDatabase : all) {
            if (iDatabase.getID().equalsIgnoreCase(db)) {
                it = iDatabase;
                break;
            }
        }
    ...
    public void run() {
        try {
            BufferedReader in = new BufferedReader(new InputStreamReader(fSocket.getInputStream(), StandardCharsets.UTF_8));
            String inputLine;
            processCommand(null);
